...
export default function ContextMenu({ x, y, node, onClose, onExpand, onFocus }: ContextMenuProps) {
  ...
  const isDocument = ['Document', 'SOP', 'P&ID', 'Inspection', 'Manual'].includes(node.type);
  ...
  return (
    ...
    >
      <div className="px-4 py-3 border-b border-slate-800/50 flex items-center justify-between bg-slate-800/30">
      ...
      <div className="py-1">
        {isDocument ? (
          <button onClick={() => onClose()} className="w-full px-4 py-2 text-left text-xs text-slate-300 hover:bg-slate-800 hover:text-white flex items-center gap-2 transition-colors">
